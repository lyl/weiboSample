...
#define WM_OAUTH_SUCCESS   (WM_APP + 1)
...
// Event map and handlers
  ...
  #pragma warning(default:4867)
	...
	void __stdcall OnEventDocumentComplete(IDispatch* /*pDisp*/, VARIANT* URL)
	{
		...
		if (pos >= 0)
		{
			int pos = strUrl.Find(_T("code="));
			SendMessage(GetTopLevelWindow(), WM_OAUTH_SUCCESS, (WPARAM)m_hWnd, (LPARAM)OLE2T(URL->bstrVal));
		}
...
__declspec(selectany) _ATL_FUNC_INFO CBrowserView::BeforeNavigate2_Info = { CC_STDCALL, VT_EMPTY, 7,
